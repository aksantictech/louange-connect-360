import { ShieldCheck, Users } from "lucide-react";
import PageHeader from "@/components/layout/PageHeader";
import StatCard from "@/components/dashboard/StatCard";
import { supabase } from "@/lib/supabase/client";

export const dynamic = "force-dynamic";

export default async function ServantsPage() {
  const { data: servants } = await supabase
    .from("servants")
    .select("*, cells(name, code, country, city)")
    .order("created_at", { ascending: false });

  const list = servants || [];

  const active = list.filter((item: any) => item.status === "active").length;

  const departments = new Set(
    list.map((item: any) => item.department).filter(Boolean)
  ).size;

  return (
    <div>
      <PageHeader
        title="Serviteurs"
        description="Liste des serviteurs, départements et responsabilités rattachées aux cellules."
      />

      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <StatCard
          title="Serviteurs"
          value={String(list.length)}
          subtitle="Total enregistré"
          icon={Users}
          tone="purple"
        />

        <StatCard
          title="Actifs"
          value={String(active)}
          subtitle="En service"
          icon={ShieldCheck}
          tone="green"
        />

        <StatCard
          title="Départements"
          value={String(departments)}
          subtitle="Services distincts"
          icon={ShieldCheck}
          tone="gold"
        />
      </section>

      <section className="mt-6 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
        <h3 className="text-xl font-black text-gray-950">
          Liste des serviteurs
        </h3>

        <div className="mt-5 overflow-hidden rounded-2xl border border-gray-100">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-100">
              <thead className="bg-[var(--louange-purple-dark)] text-white">
                <tr>
                  <Th>Nom</Th>
                  <Th>Rôle</Th>
                  <Th>Département</Th>
                  <Th>Cellule</Th>
                  <Th>Contact</Th>
                  <Th>Statut</Th>
                </tr>
              </thead>

              <tbody className="divide-y divide-gray-100 bg-white">
                {list.map((servant: any) => (
                  <tr key={servant.id}>
                    <Td>{servant.full_name}</Td>
                    <Td>{servant.servant_role}</Td>
                    <Td>{servant.department || "-"}</Td>
                    <Td>
                      {servant.cells
                        ? `${servant.cells.name} (${servant.cells.code})`
                        : "Non rattaché"}
                    </Td>
                    <Td>
                      <p>{servant.phone || "-"}</p>
                      <p className="text-xs text-gray-500">
                        {servant.email || ""}
                      </p>
                    </Td>
                    <Td>{servant.status}</Td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      </section>
    </div>
  );
}

function Th({ children }: { children: React.ReactNode }) {
  return (
    <th className="whitespace-nowrap px-4 py-4 text-left text-xs font-black uppercase tracking-wide">
      {children}
    </th>
  );
}

function Td({ children }: { children: React.ReactNode }) {
  return (
    <td className="whitespace-nowrap px-4 py-4 text-sm font-semibold text-gray-700">
      {children}
    </td>
  );
}